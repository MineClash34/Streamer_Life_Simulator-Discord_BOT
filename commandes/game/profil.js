...
exports.run = async (message, Lang, args, getRandomColor, client, addEmoji, getProfilElement, Setup) => {
    ...
    .addField(`${addEmoji("stats")} ${Lang.Stats}`, `**•》** Stream : \`${await getProfilElement("Stream", message.author.id)}\`\n**•》** ${Lang.Up} : \`${await getProfilElement("Sleep", message.author.id)}\`\n**•》** ${Lang.Views} : \`${await getProfilElement("Views", message.author.id)}\``, true)
